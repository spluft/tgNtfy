...
func (c *Coalescer) Add(ctx context.Context, key Key, item *Item) {
	...
	if !ok {
		...
		p.timer = time.AfterFunc(c.window, func() {
			...
			c.mu.Unlock()
			// The ctx passed to Add() is the HTTP request context, which is
			// already canceled by the time the window fires. Flush with a live
			// context (same as forceFlush) or the delivery row insert fails with
			// "context canceled" and the message is never delivered.
