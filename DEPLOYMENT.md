...
services:
  tgntfy:
    ...
    environment:
      ...
      - TG_BOT_SOCKS5_PROXY_URL=socks5://192.168.1.200:9000  # host has NO direct Telegram egress
      - LISTEN_ADDR=:8080
...
- **SOCKS5 egress (`TG_BOT_SOCKS5_PROXY_URL`):** default empty = direct connection to
  `api.telegram.org`. The Proxmox host (192.168.1.200) has **no** direct egress to
  Telegram, so the deployed container must set
  `TG_BOT_SOCKS5_PROXY_URL=socks5://192.168.1.200:9000` (xray SOCKS5, no auth) — the
  same route gorecomendarr's telegram-bot uses via `TELEGRAM_BOT_SOCKS5_PROXY_URL`.
  When unset, behavior is byte-for-byte the previous direct/mock path; a malformed
  value fails startup with a clean error naming the variable.
